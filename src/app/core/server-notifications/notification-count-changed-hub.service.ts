...
export class NotificationCountChangedHubService {
    private hubConnection?: signalR.HubConnection;
    notificationCountChanged$ = new BehaviorSubject<number>(0);

    private get url(): string {
        return '/api/rpc/notificationCountChangedHub';
    }
    ...
    close(): Promise<void> {
        ...
        this.hubConnection.off('Notification Count Changed');
    ...
    private registerEvents(hubConnection: signalR.HubConnection): void {
        hubConnection.on('NotificationCountChanged', (count) => {
          console.log(count);
          this.notificationCountChanged$.next(count);
        });
    }
